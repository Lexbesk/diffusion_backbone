...
from diffuser_actor.utils.cross_view_attention import CrossViewPAnet
...
class DenoiseActor(nn.Module):
    ...
    def __init__(self,
        ...
        self.scene_contextualizer = CrossViewPAnet(
    ...
    def encode_inputs(self, visible_rgb, visible_pcd, instruction,
        ...
        _, ncam, _, h, w = context_feats.shape
        context = einops.rearrange(
            pcd_pyramid[0],
            "b (ncam h w) c -> b ncam h w c", ncam=ncam, h=h, w=w
        )
        # context_feats = self.scene_contextualizer(context_feats, context)
        ...
        context_feats, context = self.encoder.run_fps(
            context_feats.transpose(0, 1),
            self.encoder.relative_pe_layer(context)
        )
        context_feats = context_feats.transpose(0, 1)
        # context = self.encoder.relative_pe_layer(context)
...
class TransformerHead(nn.Module):
    ...
    def __init__(self,
        ...
        self.cross_attn = ParallelAttention(
            num_layers=6, d_model=embedding_dim, n_heads=num_attn_heads,
            self_attention1=False, self_attention2=False,
        ...
        self.rotation_attn = FFWRelativeCrossAttentionModule(
        ...
        self.position_attn = FFWRelativeCrossAttentionModule(
            embedding_dim, num_attn_heads, 2, use_adaln=True
        )
    ...
    def prediction_head(self,
        ...
        #     diff_ts=time_embs
        # )[-1]
        gripper_features, context_features = self.cross_attn(
            gripper_features.transpose(0, 1), None,
            context_features.transpose(0, 1), None,
            seq1_pos=rel_gripper_pos, seq2_pos=rel_context_pos,
            ada_sgnl=time_embs
        )
        gripper_features = gripper_features.transpose(0, 1)
        context_features = context_features.transpose(0, 1)

        # Rotation head
        rotation = self.predict_rot(
            gripper_features, context_features,
            rel_gripper_pos, rel_context_pos, time_embs, instr_feats
        )
    ...
    def predict_pos(self, gripper_features, context_features,
                    rel_gripper_pos, rel_context_pos, time_embs,
                    instr_feats):
        position_features = self.position_attn(
            query=gripper_features,
            value=context_features,
            query_pos=rel_gripper_pos,
            value_pos=rel_context_pos,
    ...
    def predict_rot(self, gripper_features, context_features,
                    rel_gripper_pos, rel_context_pos, time_embs,
                    instr_feats):
        rotation_features = self.rotation_attn(
            query=gripper_features,
            value=context_features,
            query_pos=rel_gripper_pos,
            value_pos=rel_context_pos,
