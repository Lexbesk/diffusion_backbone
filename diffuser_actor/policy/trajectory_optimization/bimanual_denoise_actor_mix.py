...
from .denoise_actor_mix import DenoiseMixActor, TransformerHead
from diffuser_actor.utils.utils import normalise_quat


class BimanualDenoiseMixActor(DenoiseMixActor):
    ...
    def encode_inputs(self, visible_rgb, visible_pcd, instruction,
        ...
        return (curr_gripper, gripcam_context_feats, gripcam_context,
                non_gripcam_context_feats, non_gripcam_context, instr_feats,
                adaln_gripper_feats, non_gripcam_fps_feats, non_gripcam_fps_pos)
...
class BimanualTransformerHead(TransformerHead):
    ...
    def forward(self, trajectory_feats, trajectory, timestep,
        ...
        Arguments:
            ...
            non_gripcam_fps_feats: (N, B, F), N < non_gripcam_context_feats.size(1)
        ...
        traj_time_pos = self.traj_time_emb(
            torch.arange(0, traj_len + 1, device=traj_feats.device)
        )  # Set gripcam to time=0
        traj_time_pos = torch.cat([
            traj_time_pos[:1].repeat(num_gripcam, 1),
            traj_time_pos[1:].repeat(nhand, 1)
        ], dim=0)
        traj_time_pos = traj_time_pos[None, None].repeat(len(traj_feats), 1, 1, 1)
