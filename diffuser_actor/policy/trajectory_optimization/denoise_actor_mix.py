...
class GripcamEncoder(Encoder):
    ...
    def __init__(self,
        ...
        self.gripcam_proj = nn.Conv2d(2048, embedding_dim, 1)

    def encode_gripcam_images(self, rgb):
        ncam = rgb.shape[1]
        # Pass each view independently through backbone
        rgb = einops.rearrange(rgb, "bt ncam c h w -> (bt ncam) c h w")
        rgb = self.normalize(rgb)
        rgb_features = self.backbone(rgb)["res5"]
        rgb_features = F.adaptive_avg_pool2d(rgb_features, 1)
        rgb_features = self.gripcam_proj(rgb_features)

        rgb_features = einops.rearrange(
            rgb_features, "(bt ncam) c h w -> bt ncam c h w", ncam=ncam
        )

        return rgb_features


class DenoiseMixActor(nn.Module):
    ...
    def __init__(self,
        ...
        # Camera ids
        self.camera_ids = nn.Embedding(1, embedding_dim)

    def encode_non_gripcam_inputs(self, rgb, pcd, instruction, curr_gripper):
        # Compute visual features/positional embeddings at different scales
        rgb_feats_pyramid, pcd_pyramid = self.encoder.encode_images(rgb, pcd)
    ...
    def encode_gripcam_inputs(self, rgb, pcd, curr_gripper):
        ncam = rgb.shape[1]
        context_feats = self.encoder.encode_gripcam_images(rgb)

        h, w = context_feats.shape[-2:]
        dtype = context_feats.dtype
        ...
        context = curr_gripper

        return (context_feats, curr_gripper)

    def encode_inputs(self, visible_rgb, visible_pcd, instruction,
                      curr_gripper, is_gripper_camera):
        non_gripcam_inputs = self.encode_non_gripcam_inputs(
            visible_rgb[:, ~is_gripper_camera],
            visible_pcd[:, ~is_gripper_camera],
            instruction,
            curr_gripper
        )
        non_gripcam_inputs = [i for i in non_gripcam_inputs]
        gripcam_inputs = self.encode_gripcam_inputs(
            visible_rgb[:, is_gripper_camera],
            visible_pcd[:, is_gripper_camera],
            curr_gripper
        )
        gripcam_inputs = [i for i in gripcam_inputs]

        return gripcam_inputs + non_gripcam_inputs

    def policy_forward_pass(self, trajectory, timestep, fixed_inputs):
        # Parse inputs
        (
            gripcam_context_feats,
            gripcam_context,
            non_gripcam_context_feats,
            non_gripcam_context,
            instr_feats,
            adaln_gripper_feats,
            non_gripcam_fps_feats,
            non_gripcam_fps_pos
        ) = fixed_inputs
        ...
        return self.prediction_head(
            trajectory,
            timestep,
            gripcam_context_feats=gripcam_context_feats,
            gripcam_context=gripcam_context,
            non_gripcam_context_feats=non_gripcam_context_feats,
            non_gripcam_context=non_gripcam_context,
            instr_feats=instr_feats,
            adaln_gripper_feats=adaln_gripper_feats,
            non_gripcam_fps_feats=non_gripcam_fps_feats,
            non_gripcam_fps_pos=non_gripcam_fps_pos
        )
...
class TransformerHead(nn.Module):
    ...
    def forward(self, trajectory, timestep,
        ...
        Arguments:
            ...
            timestep: (B, 1)
            gripcam_context_feats: (B, N, F)
            gripcam_context: (B, N, 3+6+x)
            non_gripcam_context_feats: (B, N, F)
            non_gripcam_context: (B, N, F, 2)
            instr_feats: (B, max_instruction_length, F)
            adaln_gripper_feats: (B, nhist, F)
            non_gripcam_fps_feats: (N, B, F), N < context_feats.size(1)
            non_gripcam_fps_pos: (B, N, F, 2)
        """
        # Trajectory features
        ...
        non_gripcam_context_feats = einops.rearrange(
            non_gripcam_context_feats, 'b l c -> l b c')
        ...
        pos_pred, rot_pred, openess_pred = self.prediction_head(
            ...
            non_gripcam_context[..., :3], non_gripcam_context_feats,
