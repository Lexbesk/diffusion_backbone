...
class DenoiseActorObjectCentric(nn.Module):
    ...
    def __init__(self,
        ...
        self.obj0_linear = nn.Linear(384, embedding_dim) # to match the feature dim
        self.obj1_linear = nn.Linear(384, embedding_dim) # to match the feature dim


    def encode_inputs(
        self, 
        rgb_feature, # B,2,n_pts,dimo_feature_channel_dim
        object_pcd,  # B,2,n_pts,3 
        instruction,
        curr_gripper
    ):
        n_pts = rgb_feature.shape[2]

        # Object-centric features
        obj0_feature = rgb_feature[:, 0]  # (B, n_pts, embedding_dim)
        obj1_feature = rgb_feature[:, 1]  # (B, n_pts, embedding_dim)
        obj0_context_feats = self.obj0_linear(obj0_feature)  # (B, n_pts, emb_dim)
        obj1_context_feats = self.obj1_linear(obj1_feature)  # (B, n_pts, emb_dim)

        # Concatenate objects to form point cloud
        context_feats = torch.cat([obj0_context_feats, obj1_context_feats], 1)
        context = object_pcd.flatten(1, 2)  # (B,2*n_pts,3)
        ...
        # Encode active object pose (B, nhist, F)
        ...
        obj0_fps_feats, obj0_fps_pos = self.encoder.run_fps(
            context_feats[:, :n_pts].transpose(0, 1),
            self.encoder.relative_pe_layer(context[:, :n_pts])
        )
        obj1_fps_feats, obj1_fps_pos = self.encoder.run_fps(
            context_feats[:, n_pts:].transpose(0, 1),
            self.encoder.relative_pe_layer(context[:, n_pts:])
        )
        fps_feats = torch.cat([obj0_fps_feats, obj1_fps_feats])
        fps_pos = torch.cat([obj0_fps_pos, obj1_fps_pos], 1)
...
class TransformerHead(nn.Module):
    ...
    def __init__(self,
        ...
        # Object embedding ids
        self.obj0_emb = nn.Embedding(1, embedding_dim) # for dino features
        self.obj1_emb = nn.Embedding(1, embedding_dim) # for dino features
    ...
    def prediction_head(self,
        ...
        n_pts = context_features.shape[0]
        gripper_features = self.cross_attn(
            ...
            seq2_sem_pos=torch.cat([
                self.obj0_emb.weight.repeat(n_pts // 2, 1)[None],
                self.obj1_emb.weight.repeat(n_pts // 2, 1)[None]
            ], 1),
            ada_sgnl=time_embs
        )[-1].transpose(0, 1)

        # Self attention among gripper and sampled context
        n_pts = sampled_context_features.shape[0]
        ...
        features = self.self_attn(
            ...
            seq1_pos=rel_pos,
            seq1_sem_pos=torch.cat([
                torch.zeros(1, len(gripper_features), gripper_features.shape[-1]).to(features.device),
                self.obj0_emb.weight.repeat(n_pts // 2, 1)[None],
                self.obj1_emb.weight.repeat(n_pts // 2, 1)[None]
            ], 1),
            seq2_pos=rel_pos,
    ...
    def predict_pos(self, features, rel_pos, time_embs, num_gripper,
                    instr_feats):
        n_pts = features[num_gripper:].shape[0]
        position_features = self.position_self_attn(
            seq1=features.transpose(0, 1),
            seq2=features.transpose(0, 1),
            seq1_pos=rel_pos,
            seq1_sem_pos=torch.cat([
                torch.zeros(1, num_gripper, features.shape[-1]).to(features.device),
                self.obj0_emb.weight.repeat(n_pts // 2, 1)[None],
                self.obj1_emb.weight.repeat(n_pts // 2, 1)[None]
            ], 1),
            seq2_pos=rel_pos,
