...
from .rot_util import (
    np_get_delta_qpos,
    np_normalize_vector,
)
from .hand_util import MjHO
from .file_util import load_json
from .fc_metric import *


class BaseEval:
    def __init__(self, input_npy_path, configs, num, visualize):
        
        self.configs = configs
        self.num = num
        # check if input_npy_path is str or dict
        if isinstance(input_npy_path, str):
            self.grasp_data = np.load(input_npy_path, allow_pickle=True).item()
        else:
            self.grasp_data = input_npy_path
        ...
        self.mj_ho = MjHO(
            ...
            debug_render=visualize,
            debug_viewer=False,
            configs=configs,
        )
        
        self.configs.task.debug_render = visualize
    ...
    def _eval_pene_and_contact(self):
        ...
        print("Evaluating penetration and contact...")
        # print('grasp_qpos: ', self.grasp_data["grasp_qpos"], 'obj_pose: ', self.grasp_data["obj_pose"])
