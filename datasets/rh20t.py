    ...
    def _load_instructions(self, instruction_file):
        ...
        with open( instruction_file, "r", encoding="utf-8") as file:
            ...
            for line in lines:
                ...
                task_name = description.replace(' ', '_')
                # print("task_name: ", task_name)
                # print("description: ", description)
                self.task_list.append( task_name )
                variance_dict = {"0": [description] }      
                task_dict[task_name] = variance_dict
        self.task_dict = copy.deepcopy(task_dict)

        return task_dict

    def _get_task(self, idx):
        return [self.tasks[int(self.annos['task_id'][idx])]] # todo???
