...
from .utils import compute_metrics_action_objpose as compute_metrics
from types import SimpleNamespace
from datasets.base_dex import DexDataset
from mujoco_visualization import val_batch
from datasets.dexterousact import make_collate_train, make_collate_eval



class DexterousActTrainTester:
    ...
    def prepare_batch(self, sample):
        out = {}
        out['q_hist'] = sample['q_hist'].cuda(non_blocking=True).float()
        out['v_hist'] = sample['v_hist'].cuda(non_blocking=True).float()
        out['ee_fingers'] = sample['ee_fingers'].cuda(non_blocking=True).float()
        out['obj_pose_hist'] = sample['obj_pose_hist'].cuda(non_blocking=True).float()
        out['act_hist'] = sample['act_hist'].cuda(non_blocking=True).float()
        out['goal_pos'] = sample['goal_pos'].cuda(non_blocking=True).float()
        ...
        out['object_asset'] = sample['object_asset']
        out['obj_init_pcl_cam'] = sample['obj_init_pcl_cam'].cuda(non_blocking=True).float()
        
        # print(out['depth_hist'].shape, 'depth hist shape')
        # print(out['q_hist'].shape, 'q hist shape')
        # print(out['v_hist'].shape, 'v hist shape')
        # print(out['ee_fingers'].shape, 'ee fingers shape')
        # print(out['obj_pose_hist'].shape, 'obj pose hist shape')
        # print(out['act_hist'].shape, 'act hist shape')
        # print(out['goal_pos'].shape, 'goal pos shape')
        # print(out['grasp_cond'].shape, 'grasp cond shape')
        # print(out['intrinsics'].shape, 'intrinsics shape')
        # print(out['extrinsics'].shape, 'extrinsics shape')
        # if 'obj_pose_future' in out:
        #     print(out['obj_pose_future'].shape, 'obj pose future shape')
        #     print(out['act_future'].shape, 'act future shape')
        # print(out['init_seg'].shape, 'init seg shape')
        # print(out['object_scale'].shape, 'object scale shape')
        # print(out['obj_init_pcl_cam'].shape, 'obj init pcl cam shape')
        # print('done preparing batch')
    ...
    def evaluate_nsteps(self, model, loader, step_id, val_iters, split='val'):
        ...
        for i, sample in tqdm(enumerate(loader)):
            ...
            sample = self.prepare_batch(sample)
            action_future, q_future, obj_pose_future = self._model_forward(model, sample, training=False)
            ...
            gt_action_future = sample["act_future"]
            gt_obj_pose_future = sample["obj_pose_future"]
            gt_q_future = sample["q_future"]
            
            sample_to_save = {}
            
            losses, losses_B = compute_metrics(action_future, gt_action_future, q_future, gt_q_future, obj_pose_future, gt_obj_pose_future)
        ...
        return -values[f'{split}-losses/mean/action_acc_001']
